class Codec {
  ...
  Map<String, dynamic> makeRtp() =>
      {'payload': payload, 'codec': name, 'rate': rate, 'encoding': encoding};
}
...
class SdpSimplified {
  ...
  SdpSimplified.fromSdpMap(Map<String, dynamic> sdpMap) {
    ...
    if (indexOfAudioMedia != null && indexOfAudioMedia != -1) {
      ...
      (audioRtp as List).forEach((rtp) {
        var codec = AudioCodec(rtp['codec'], rtp['payload']);
    ...
    if (indexOfVideoMedia != null && indexOfVideoMedia != -1) {
      ...
      (videoRtp as List).forEach((rtp) {
        var codec = VideoCodec(rtp['codec'], rtp['payload']);
        codec.rate = rtp['rate'];
        codec.encoding = rtp['encoding'];
  ...
  void log() {
    print('SdpSimplified:');
    print('audioCodecs:');
    audioCodecs.forEach((element) {
      element.log();
    });
    print('videoCodecs:');
    videoCodecs.forEach((element) {
      element.log();
    });
